...
- [ ] 1.1 Bez env: integration skip, exit 0
- [ ] 1.2 Z env: unpublished A absent z `listPublishedEvents(B)`
- [ ] 1.3 Z env: published A present (positive control)
- [ ] 1.4 Z env: raw RLS SELECT unpublished A as B → null
- [ ] 1.5 Z env: self-exclude — własne published absent z list A
- [ ] 1.6 `npm test` (unit) zielone bez env

#### Ręczne

- [ ] 1.7 Seed published ma `published_at` + triage accepted/maybe; brak wholesale Supabase mock
